...
import time
from my_yolo_interface import yolo_workouts
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def bind(self):
        ...
        self.pushButton_stop.clicked.connect(self.stop_video_camera)
        self.pushButton_camera.clicked.connect(self.startCamera)
        self.pushButton_resetcount.clicked.connect(self.set_count_offset)
    ...
    def init_sth(self):
        ...
        初始化一些自己的东西
        '''
        self.current_count = 0  # 计数器
        self.count_offset = 0  # 计数偏移量,可以实现次数清零
        self.statusBar().showMessage("状态：")
        self.cap = None  # 初始化cap属性为None
        self.video_path = ""   # 初始化视频路径
        self.set_filepath()
        self.file_is_playing = True  # 默认状态设置为播放
        self.video_writer = None  # 初始化视频写入对象

    def init_model(self):
        '''
        初始化模型
        '''
        currentModel = self.comboBox_model.currentText()
        currentMotion = self.comboBox_motion.currentText()
        self.yolo11_model = yolo_workouts()
        self.yolo11_model.set_model_video(currentModel, self.video_path)
        self.yolo11_model.set_motion_type(currentMotion, show=False)
        print(f'==debug==>初始化模型成功')
        print(f'currentModel: {currentModel}, currentMotion: {currentMotion}')
    ...
    def set_count_offset(self):
        '''
        设置计数偏移量，实现次数清零
        '''
        self.count_offset = self.current_count
    ...
    def start_video(self):
        ...
        播放视频
        '''
        self.stop_video_camera()  # 先停止播放视频和摄像头
        print("==debug==>播放视频")
        ...
        ratio2 = height / self.video_input.height()  # (label 高度)
        ratio = max(ratio1, ratio2)
        # print(f'==debug==>width: {width}, height: {height},video_input.width(): {self.video_input.width()},' )
        print(f'==debug==>ratio1: {ratio1}, ratio2: {ratio2}, ratio: {ratio}')
        self.file_is_playing = True  # 默认状态设置为播放

        # 视频输出路径
        out_path = "./runs/" + "yolo_output_" + \
            os.path.splitext(os.path.basename(self.video_path))[0] + ".avi"
        # 创建一个视频写入对象，用于将处理后的视频保存为.avi文件
        if self.radioButton_saveoutput.isChecked():
            self.video_writer = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc(
                *"mp4v"), frame_rate, (width, height))
            print("==debug==>创建视频写入对象")
        else:
        ...
        while video.isOpened() and self.file_is_playing:
            ...
            # 显示原始视频
            # 将图片转换为 Qt 格式 QImage:QImage(bytes,width,height,format)
            ...
            # 显示处理后的视频# yolo11前向计算
            self.show_yolo_frame(frame=frame_rgb, width=width, height=height,
                                 ratio=ratio)
            cv2.waitKey(5)
            # delay = 1000 / 60, cv2.waitKey(int(delay))  # 60 FPS
    ...
    def frameAnalyzeThreadFunc(self):
        while True:
            ...
            # 把读到的帧的大小重新设置
            frame = cv2.resize(frame, (640, 480))
            self.show_yolo_frame(
                frame=frame, width=640, height=480, ratio=1)
            # time.sleep(0.5)

    def show_yolo_frame(self, frame, width, height, ratio):
        # print(f'debug==>show_yolo_frame: {frame.shape} frame.dtype: {frame.dtype}')
        # 显示 yolo11 的输出结果
        yolo_output, angle_data, count_data, stage_data = self.yolo11_model.detect_image(
            frame)
        print(
            f'==debug==>angle_data: {angle_data}, count_data: {count_data}, stage_data: {stage_data}')
        if count_data is None or len(count_data) == 0:
            print("==debug==>count_data is None or empty")
            return
        self.current_count = count_data[0]
        self.label_angel.setText(f"角度: {angle_data[0]:.2f}")
        self.label_count.setText(f"计数: {count_data[0]-self.count_offset}")
        # self.label_stage.setText(f"阶段: {stage_data[0]}")
        self.label_stage.setText(
            "阶段: 上" if stage_data[0] == 'up' else "阶段: 下" if stage_data[0] == 'down' else f"阶段: {stage_data[0]}")
        ...
        if self.radioButton_saveoutput.isChecked():  # 保存视频
            self.video_writer.write(cv2.cvtColor(yolo_output, cv2.COLOR_RGB2BGR))  # 将处理后的帧写入输出视频
